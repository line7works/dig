...
- Works only on playlists the connected account owns or collaborates on. Friends' playlists and Spotify editorial lists (Discover Weekly etc.) are closed to this app type — that is Spotify's rule.
...
- Reads are context-disciplined: results are compact by default (pass detail:"standard"|"full" for more), and lists are paginated. When a reply names a next call, use exactly that call to continue; when it says it reached the end, stop. For "how many / which ones" questions, use dig_find_in_playlist or dig_diff_playlists — they answer with counts plus a sample instead of full listings, and say when a sample is truncated.
...
- All Spotify calls run through one serialized queue — never assume parallel calls are faster; issue them one at a time. If a result says Spotify asked for a wait (rate limit), stop calling and tell the user how long; retrying sooner makes the wait grow.
- Writes are additive only (create, add, rename, reorder). dig_add_tracks verifies every proposal first: uncertain matches come back as questions, NEVER added — relay them, report misses honestly, never pad a list.
- Every write reports verified | accepted | ambiguous | partial from a re-read. Anything but verified: re-read before acting; never blind-retry an add.
