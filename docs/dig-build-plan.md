...
### 2026-08-16 · Slice G2 live AC2 (Tony, desktop app)
- AC2 PASSED live 2026-08-16: in one fresh desktop-app chat — dig_status showed "not configured" despite the Client ID sitting in settings.json (the desktop userConfig gap re-proven on 0.2.0), then paste-in-chat → dig_set_client_id ("stored and active"), dig_connect → browser approval as Pour Guys, dig_status showed signed in with Source: Dig's config file (set in chat), and dig_list_playlists returned all 14 playlists live. No new chat needed anywhere after install
- The desktop app assigns a DIFFERENT plugin data directory than terminal sessions: `~/.claude/plugins/data/dig-inline` vs `dig-dig` — desktop and terminal keep separate config.json/token stores; a user who sets up in one surface starts unconfigured in the other. Slice H's README/support story should know this
- Plugin version bumped to 0.2.0 post-signoff to release G2: `claude plugin update` keys on the version number and refused to refresh the cached 0.1.0 snapshot (the cache pins commit + version) · per user
